...
import { UBI_APPID, UBI_SESSIONID, BASE_UBI_URI, UBI_RANKED_URI_V2, GetRanksById, } from '../constants';
export const GetUserRank = async (userId, platform) => {
    if (!userId || !platform)
    ...
    const result = await extractValues(await response);
    return result;
};
const extractValues = async (profiles) => {
    const ranks = {};
